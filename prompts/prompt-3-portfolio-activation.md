...
prompt_name: prompt-3-portfolio-activation
prompt_hash: 3ab187c353f616aa0b629e20719f5415463ea36b76cda89d89734a95557eeb22
model: gpt-4o
last_updated: 2026-05-27
-->

# Prompt 3v2 — Field-of-Play Activation Plan + Move Artefacts + Network Toolkit

**Pipeline position:** Prompt 3v2 of 10 (replaces Prompt 3 for field-of-play planning)
**Runs after:** User selects 5 strands (field of play) from the top 10 options
**Inputs:** Selected strand options from Prompt 1 (with primary_move_type per strand) + Prompt 2 hook_insight + Q2, Q3b, Q5, Q6, Q11, Q12, Q13, Q14 + cv_extract (if present)
**Output:** 30-Day Field-of-Play Activation Plan (integrated across all strands) + Move Artefacts per task (type-appropriate) + Network Toolkit
**Replaces:** `prompt-3-activation.md` (single-option version)
**Version:** 2.3 — April 2026 (Apollo sprint — outreach_subtype and apollo_query fields added to outreach task schema; two new QUALITY RULES: OUTREACH SUBTYPE REQUIRED and APOLLO QUERY FOR COLD TASKS)
...
At runtime:
1. Take the selected strand options (2–5) from the report's `selected_strands` array
2. Take the `hook_insight` from Prompt 2 output — both `headline` and `insight` fields
3. Build the strand context array (each strand's model_name, positioning, target_buyer, pricing, difficulty_rating)
4. Inject alongside Q2, Q3b, Q5, Q6, Q11, Q12, Q13, Q14, hook_insight, and cv_extract (if cv_uploaded = true) into the user message
5. Send to the API
6. Store the full output as `activation_plan` in the report record. The `first_move` is surfaced to the user **before** the full plan is shown.

**Recommended model:** `gpt-5.4`
**Temperature:** `0.5`
**Max tokens:** `16384` (gpt-4o model ceiling — 22000 exceeded the limit and caused 400 errors. Phase 2/4 truncation fixed by compression rule: outreach_draft bodies in phase tasks abbreviated to 60–80 words; full templates remain in network_toolkit)
...
```
You are Solo's field-of-play activation specialist. Your job is to take a user's chosen field of play — 5 business model strands they want to explore in parallel — and produce:
...
```
Here is the user's selected opportunity portfolio:
