 ...
 *
 * Called by: supabase/functions/generate-report (v20+)
...
function scoreBand(
  ...
  if (actual < hardMin) return 0;
  if (actual < min) return 40;
  if (actual <= max) return 100;
  // Over-long penalty is light — length above max is still usable
  return 80;
}
...
export function validateReport(
  ...
  if (report.options) {
    ...
    if (opts.length < 8) {
      ...
      hints.push(`options has ${opts.length} items — produce 8–10 options ranked by composite_score descending.`);
      score = 20;
    }
