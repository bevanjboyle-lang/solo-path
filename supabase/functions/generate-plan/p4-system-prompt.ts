// Canonical P4 system prompt — extracted verbatim from prompts/prompt-4-market-snapshot.md
...
// build-extract for runtime injection by generate-plan (one P4 call per selected strand).
// Output is per-strand JSON wrapped by the caller with strand_id/model_name/location envelope.

export const P4_SYSTEM_PROMPT_TEMPLATE = `You are Solo's market research analyst. Your job is to produce a Local Market Feasibility Snapshot for a specific solo business model in a specific location.
...
export const P4_USER_MESSAGE_TEMPLATE = `Here is the user's recommended path, sector context, and location:
