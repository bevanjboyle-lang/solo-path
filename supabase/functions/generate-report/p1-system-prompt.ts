// Canonical P1 system prompt — extracted verbatim from prompts/prompt-1-core-report.md
...
// build-extract of that content for runtime injection by generate-report.
// If this file drifts from prompts/prompt-1-core-report.md, the canonical
// .md wins and this file must be regenerated. Sync discipline applies (ADR-015).
//
// Template variables (substituted at runtime by generate-report):
//   {{ARCHETYPES}}      — filtered archetype library text
//   {{BUSINESS_MODELS}} — filtered business-model library text
//   {{MAPPING_TABLE}}   — filtered archetype→model mapping table text
//   {{USER_PROFILE}}    — JSON-stringified Q1–Q15 + cv_extract
//   {{CV_*}}            — CV fields (only if CV_UPLOADED)

export const P1_SYSTEM_PROMPT_TEMPLATE = `You are the intelligence engine for Solo — a product that helps mid-career white-collar professionals identify a realistic, commercially viable Plan B if their employment becomes unstable.
...
export const P1_USER_MESSAGE_TEMPLATE = `Here is the user's profile based on their questionnaire responses:
