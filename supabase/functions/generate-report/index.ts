...
serve(async (req) => {
  ...
  try {
    ...
    // âââ PROMPT 1: Core Report âââ
...
5. Return JSON with this exact structure. IMPORTANT: model_name MUST be the human-readable "name" field from BUSINESS_MODELS (e.g. "Process Improvement Consultant"), NEVER the "id" field (e.g. "BM_PROCIM").
{
  ...
  "first_steps": [string, string, string, string, string]
    ...
    // âââ PROMPT 2: Evaluation âââ
    const p2System = `You are a senior commercial critic. Evaluate the Solo report against 6 criteria:
1. Specificity â target_buyer must name type/size/situation
2. Commercial realism â pricing and timelines plausible for UK market
3. Option diversity â 3 options must not share same category or sales motion
4. Recommendation quality â specific reason tied to this user
5. Reality check honesty â archetype-specific failure modes, GBP figures in income outlook
6. First steps quality â all 5 specific and tied to recommended model

Additionally, generate a "hook_insight" â a single punchy sentence of 8â12 words that captures the most commercially compelling insight about this user's independent potential. Use their industry experience (Q6), reputation (Q7), client/sector knowledge (Q11), and any independent work history (Q12) to craft this. It should read like a headline that would make the user want to see their full report.
...
Hard constraint: NEVER change a model_name value.`;
    ...
    // âââ PROMPTS 3 & 4 in parallel âââ
...
- Employed full-time: 1â1.5h weekday evenings, 3â4h weekend days
- Unemployed or in notice: 5â6h weekdays
- Part-time: 2â3h weekdays

Network calibration:
- Strong (100+ contacts): ambitious referral-led targets
- Medium (30â100): moderate mix of warm and cold
- Weak (under 30): conservative, rebuild relationships first

Cover 4 phases: Foundations (Days 1â7), Network Activation (Days 8â16), Outreach (Days 17â25), Consolidation (Days 26â30).


FIRST MOVE: Before the plan, generate a first_move â the single highest-probability action for generating real-world signal within 24 hours. Must be an outreach action naming a specific type of person to contact. Include a complete, ready-to-send draft (email, LinkedIn DM, or WhatsApp). follow_up_prompt is the exact question to ask the user 24 hours later.
Return JSON:
{
  "first_move": {
  ...
  "network_toolkit": {
    ...
    const p4System = `You are Solo's market research analyst. Produce a Local Market Feasibility Snapshot. You do not have live data â label all figures as indicative.
...
[Model name] | [Location]
Prepared as indicative research â not primary market data`;
...
Pricing: Â£${recommendedOption?.pricing?.range_low_gbp || "?"} â Â£${recommendedOption?.pricing?.range_high_gbp || "?"} ${recommendedOption?.pricing?.cadence || ""}
Location: ${answers["15"] || "UK"}`;

    // âââ PROMPT 7: AI Impact & Adaptation âââ
    ...
    const p7System = `You are Solo's AI impact analyst. Your job is to write a clear, specific, commercially honest assessment of how AI will affect this user's career â both their current role and their recommended Plan B path.

THE THREE PARTS YOU MUST WRITE:

Part 1 â AI Risk to Your Current Role: Write a candid, archetype-specific assessment. Name specific pressure mechanisms, identify resilient aspects, provide honest near-term outlook.

Part 2 â AI Resilience of Your Plan B: State displacement risk honestly, explain specifically how AI could enhance this model, describe how to position as AI-enabled.

Part 3 â Your Adaptation Path: Write 3-5 specific, actionable things tied to their specific recommended business model. Name real tools and platforms. Achievable in 90 days.
