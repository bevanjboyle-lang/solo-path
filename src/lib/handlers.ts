...
/**
 * Resume a cancelled-pending subscription.
 *
 * F46 (2026-04-19): previously invoked phantom `resume-subscription` edge
 * function (never deployed). Now routes through the Stripe Customer Portal
 * per ADR-010 — the Portal is the canonical billing surface. Opens the
 * portal in a new tab where the user can undo the scheduled cancellation.
 * Return shape `{ error?: string }` preserved for existing callers.
 */
