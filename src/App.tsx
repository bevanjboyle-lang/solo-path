 ...
 * Hash-anchor deep links are respected (FAQ.tsx has its own handler).
 */
...
// App.tsx renders the Footer for every route NOT in this list.
...
// quiet on the focused funnel + auth surfaces, same logic as the Footer policy.
...
function AnimatedRoutes() {
  ...
  const hideTicker = TICKERLESS_ROUTES.some((r) => location.pathname.startsWith(r));

  return (
    <>
      {/* The Radar ticker — ADR-026 global chrome, above the masthead. */}
      {!hideTicker && <TickerBar />}
  ...
  return (
    <ErrorBoundary>
      <QueryClientProvider client={queryClient}>
        <TooltipProvider>
          {/* ADR-026 (2026-06-10): the office-photo background layer is RETIRED.
            * The page is one flat ivory editorial surface (see index.css v2.0). */}
